...
void wirelessReceivePacket(void)
{
	if (rfm_framepos>=1)
	{
		if (((rfm_framebuf[0]&0x7f)>= RFM_FRAME_MAX) || ((rfm_framebuf[0]&0x7f)<4+2) || (rfm_framepos >= RFM_FRAME_MAX))	// reject noise
		{
			// reject invalid data
			rfm_framepos=0;
			return; // !!! return !!!
			  ...
			  rfm_framepos = (rfm_framebuf[0]&0x7f);
			{
				...
				//sync packet
					...
					if (mac_ok)
					{
						...
						cli();
						RTC_timer_done &= ~_BV(RTC_TIMER_RTC);
						sei();  // do not add one second
						return;
					}
				}
				else 
