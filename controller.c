...
{
	uint8_t i = (ring_buf_temp_avgs_pos+AVGS_BUFFER_LEN-((CTL_mode_window!=0)?config.window_close_detection_time:config.window_open_detection_time)) % AVGS_BUFFER_LEN;
	int16_t min = 10000;
	int16_t max = 0;
	while (1)
	{
...
{
	if ((absErr>=((lastTempChangeErrorAbs*3)>>2)) && (absErr > (I_ERR_TOLLERANCE_AROUND_0*2)))
	{
...
static uint8_t pid_Controller(int16_t setPoint, int16_t processValue, uint8_t old_result, bool updateNow)
{
	...
	else
	{
		if (CTL_integratorBlock == 0)
		{
			...
			{
				if (((lastErrorSign != ((uint8_t)(error16>>8)&0x80))) || ((absErr==last2AbsError) && (absErr<=I_ERR_TOLLERANCE_AROUND_0))) //sign of last error16 != sign of current OR abserror around 0
				{
