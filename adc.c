...
static int16_t ring_buf[3][AVERAGE_LEN];
...
static int32_t ring_sum [3] = {0,0,0};
int16_t ring_average [3] = {0,0,0};
 ...
 *  - Motor current = ((V_ref * 1024) / ADC_Val_Ub) / R with V_ref=1.1V and R = 2.2
 ******************************************************************************/
static int16_t ADC_Get_Mot_Current(uint16_t adc)             // Get Batteriy Voltage in mV
{
	uint32_t milivolt;
	milivolt = 1126400000;		//(V_ref * 1024) * 1000000
	milivolt= milivolt / adc;
	 
	return ((int16_t) (milivolt / 2200));
}
...
static int16_t ADC_Convert_To_Degree(int16_t adc)
{
	 ...
	 */
	dummy = (int16_t) ( (((int32_t)(adc - kx))*(-TEMP_CAL_STEP)) / (int32_t)(kx_d[i]) ); 
		...
		case 5:
			{
				...
				CurrRAW = ad;
				update_ring(CURR_RING_TYPE, ADC_Get_Mot_Current(ad));
